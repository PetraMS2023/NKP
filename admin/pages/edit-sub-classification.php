...
<body>
    ...
    <div class="dcont">
        ...
        <div class="mainCont" id="main">
            ...
            <div class="pageCont">
                <div class="conta p30">
                    <h3> اضافة وتعديل التصنيفات الرئيسية </h3>
                </div>
                <div class="conta p30">
                    <?php
                    ...
                    if (isset($_GET['id'])) {
                        ...
                        $sql = "SELECT subcategory_name_ar, subcategory_name_en, main_category_id, subcategory_image FROM subcategories WHERE id = $id";
                        $result = $conn->query($sql);

                        if ($result->num_rows > 0) {
                            $row = $result->fetch_assoc();
                            $subcategory_name_ar = $row['subcategory_name_ar'];
                            $subcategory_name_en = $row['subcategory_name_en'];
                            $main_category_id = $row['main_category_id'];
                            $subcategory_image = $row['subcategory_image'];
                        } else {
                            echo "No record found";
                            $subcategory_name_ar = $subcategory_name_en = $subcategory_image = "";
                            $main_category_id = "";
                        }
                    } else {
                        $subcategory_name_ar = $subcategory_name_en = $subcategory_image = "";
                    ...
                    <form class="inputs" method="POST" action="../../backend/update/update_subcategory.php"
                        enctype="multipart/form-data">
                        <input type="hidden" name="id" value="<?php echo $id; ?>">
                        <div class="input">
                            <span>اسم التصنيف الفرعي باللغة العربية :</span>
                            <input type="text" name="subcategory_name_ar" spellcheck="false"
                                placeholder="اسم التصنيف الفرعي باللغة العربية"
                                value="<?php echo $subcategory_name_ar; ?>">
                        </div>
                        <div class="input">
                            <span>اسم التصنيف الفرعي باللغة الانجليزية :</span>
                            <input type="text" name="subcategory_name_en" spellcheck="false"
                                placeholder="اسم التصنيف الفرعي باللغة الانجليزية"
                                value="<?php echo $subcategory_name_en; ?>">
                        </div>
                        <div class="input">
                            ...
                            <select name="main_category_id" id="">
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        echo '<option value="' . $row["id"] . '"' . ($row["id"] == $main_category_id ? ' selected' : '') . '>' . $row["category_name_ar"] . '</option>';
                                    }
                                } else {
                        ...
                        <div class="input">
                            ...
                            <div class="customInput">
                                <input type="text" spellcheck="false" placeholder="imgtest.png"
                                    value="<?php echo $subcategory_image; ?>">
                                <input type="file" name="subcategory_image" accept="image/*" hidden id="file">
