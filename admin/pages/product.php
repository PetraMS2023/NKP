        ...
        <div class="mainCont" id="main">
            ...
            <div class="pageCont">
                <div class="conta p30">
                    <h3> اضافة المنتج </h3>
                    <div class="btns">
                        <a href="./add-product.php" class="saveBtn">اضافة منتج +</a>
                    </div>
                </div>
                <div class="conta p30">
                    <form class="taIn">
                    ...
                    $sql = "SELECT p.id, p.product_image, p.product_name_ar, p.product_name_en, p.product_specs, c.category_name_ar AS main_category_name, s.subcategory_name_ar AS subcategory_name
                            FROM products p
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while($row = $result->fetch_assoc()) {
                            echo '<tr>
                                    <td><img src="' . htmlspecialchars($row["product_image"]) . '" alt=""></td>
                                    <td class="mw200">' . htmlspecialchars($row["product_name_ar"]) . '</td>
                                    <td class="mw200">' . htmlspecialchars($row["product_name_en"]) . '</td>
                                    <td class="mw200">' . htmlspecialchars($row["product_specs"]) . '</td>
                                    <td class="mw200">' . htmlspecialchars($row["main_category_name"]) . '</td>
                                    <td class="mw200">' . htmlspecialchars($row["subcategory_name"]) . '</td>
                                    <td class="tools">
                                        <a href="edit-product.php?id=' . $row["id"] . '">
                                            <img src="../media/icons/edit.png" alt="" id="edit">
                                        </a>
                                        <a href="../../backend/forms/delete_product.php?id=' . $row["id"] . '" onclick="return confirm(\'هل أنت متأكد أنك تريد حذف هذا المنتج؟\')">
